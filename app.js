...
function openSettingsScreen() {
  ...
  document.getElementById('settings-interval').value = appState.notificationInterval;
  const list = document.getElementById('active-deck-list');
  list.innerHTML = '';
  appState.stacks.forEach((stack, idx) => {
    const div = document.createElement('div');
    const cb = document.createElement('input');
    cb.type = 'checkbox';
    cb.id = `active-deck-${idx}`;
    cb.checked = !!stack.isActive;
...
function saveSettingsScreen() {
  ...
  appState.stacks.forEach((stack, idx) => {
    const checked = document.getElementById(`active-deck-${idx}`).checked;
    stack.isActive = checked;
  });
  saveStacks();
  renderStacks();
  closeSettingsScreen();
...
document.getElementById('save-settings-screen-btn').addEventListener('click', saveSettingsScreen);
